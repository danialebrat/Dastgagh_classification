...
def calculate_mel(signal, sr=SAMPLE_RATE, n_fft=N_FFT, hop_length=HOP_LENGTH, n_mels=N_MELS):
...
def store_mel(filename, seg_num, mel, label):
...
def save_mel(dataset_path, n_fft=2048, hop_length=512, n_mels=N_MELS):
        ...
        if dirpath is not dataset_path:
            ...
            for f in filenames:
                ...
                for s in range(num_segments):
                    ...
                    mel = calculate_mel(signal[start_sample:finish_sample])

                    # store log_mel for segment if it has the expected length
                    store_mel(f, s, mel, label)


if __name__ == "__main__":
    save_mel(DATASET_PATH)
