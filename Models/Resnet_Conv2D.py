...
from tensorflow.keras.layers import Dense, Dropout, Conv2D, BatchNormalization, Activation, Input, add, Flatten, MaxPool2D
...
def Resnet_encoder(model, filter):
    ...
    model = Conv2D(filters=filter, kernel_size=3, padding='same')(model)
    model = Dropout(0.1)(model)
    model = BatchNormalization()(model)
    model = Activation('relu')(model)

    model = Conv2D(filters=int(filter/2), kernel_size=3, padding='same')(model)
    model = Dropout(0.2)(model)
    model = BatchNormalization()(model)
    model = Activation('relu')(model)

    # expand channels for the sum
    shortcut = Conv2D(filters=int(filter/2), kernel_size=1, padding='same')(x)
...
def build_model(input_shape):
    ...
    gap_layer = MaxPool2D(pool_size=3, strides=1)(model)
